...
let foo = Cucumber.LibParallel.empty
          ...
          |>
            (Cucumber.LibParallel._Given
              ...
              (fun group arg (foo_state_opt, outcome) ->
                Lwt_io.printl "I am in the given: I have a simple background" >>= (fun _ -> Lwt.return (bar foo_state_opt)) 
              ))
          |>
            (Cucumber.LibParallel._Then
               ...
               (fun group arg (foo_state_opt, outcome) ->
                        match foo_state_opt with
                        | Some x ->
                           (Lwt_io.printl "I am in the then: I have done the thing" ) >>= (fun _ -> Lwt.return (Some {foo = x.foo + 1}, Cucumber.Outcome.Pass))
                        | None ->
          ...
          |>
            (Cucumber.LibParallel._Given
              ...
              (fun group arg (foo_state_opt, outcome) ->
                Lwt_io.printl "I am in the other given" >>= (fun _ -> Lwt.return (bar foo_state_opt)) 
            ))
          |>
            (Cucumber.LibParallel._When
